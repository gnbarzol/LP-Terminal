...
def p_data_repeat_in_structures(p):
  '''DATA_REPEAT : DATA_ALLOWED_IN
                  | DATA_ALLOWED_IN DATA_REPEAT
  '''
  if len(p) == 1:
    p[0] = ('DATA_IN', p[1])
  else:
...
def p_data_allowed_in_structures(p):
  '''DATA_ALLOWED_IN : ASIGNATION
                          | EXPRESSION
  '''
  p[0] = ('DATA', p[1])
...
def p_data_repeat_in_structures_if(p):
  '''DATA_REPEAT_IF : DATA_ALLOWED_IN
                  | DATA_ALLOWED_IN DATA_REPEAT_IF
                  | DATA_ALLOWED_IN ELSE DATA_ALLOWED_IN
  '''
  if len(p) == 1:
    p[0] = ('DATA_IN', p[1])
  else:
...
def p_structure_initial_while(p):
